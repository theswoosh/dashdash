...
export const WIDGET_CATALOG: WidgetTemplate[] = [
  ...
  },
  {
    ...
    defaultOptions: { showCpu: true, showMem: true, showUptime: true, showDisk: true, showCpuTemp: false },
    configFields: [
      { key: 'showCpu', label: 'Show CPU', type: 'boolean', default: true },
      { key: 'showMem', label: 'Show memory', type: 'boolean', default: true },
      { key: 'showDisk', label: 'Show disk', type: 'boolean', default: true },
      { key: 'showCpuTemp', label: 'Show CPU temperature', type: 'boolean', default: false },
      { key: 'showUptime', label: 'Show uptime', type: 'boolean', default: true },
      {
        key: '_cputemp_info',
        label: 'CPU temperature is Linux-only and often unavailable inside Docker containers (host sensors usually aren\'t mounted in) — defaults off.',
        type: 'info',
      },
